...
Otherwise, if you want to run Sports2D automatically for example, you can choose other ordering methods such as 'highest_likelihood', 'largest_size', 'smallest_size', 'greatest_displacement', 'least_displacement', 'first_detected', 'last_detected', 'medicine_ball', or 'motion_specific'. The `medicine_ball` mode keeps only people present in at least 95% of frames and ranks them by distance to the selected ball during the first 10 frames. The `motion_specific` mode ranks candidates as `broad_jump`, `sprint_start`, or `etc` from their coordinate motion pattern. For `broad_jump`, it first applies whole-video gate filters for presence, confidence, and pose-bbox size, then requires an airborne interval with simultaneous horizontal motion. For `sprint_start`, it does not require whole-video presence or bbox-size gates; instead it finds each candidate's maximal hip-normalized horizontal speed frame, checks the preceding 1 second window for enough confidence, and requires fast horizontal motion plus alternating heel-y oscillation.
...
'person_ordering_method': ["", "'on_click', 'highest_likelihood', 'largest_size', 'smallest_size', 'greatest_displacement', 'least_displacement', 'first_detected', 'last_detected', 'medicine_ball', or 'motion_specific'. 'medicine_ball' keeps only the people present in at least 95% of frames and ranks them by distance to the selected ball during the first 10 frames. 'motion_specific' ranks broad_jump/sprint_start candidates. broad_jump uses whole-video presence/confidence/pose-bbox gates and requires airborne horizontal motion; sprint_start uses a local 1 s window before each track's peak hip-normalized horizontal speed, then requires fast horizontal motion with alternating heel-y oscillation. 'on_click' if not specified"],
...
'vertical_jump': ["", "run Sports2D-only vertical jump event/force analysis from 2D coordinates. false if not specified"],
...
4. **Chooses which persons to analyze.** In single-person mode, only keeps the person with the highest average scores over the sequence. In multi-person mode, you can choose the number of persons to analyze (`nb_persons_to_detect`), and how to order them (`person_ordering_method`). The ordering method can be 'on_click', 'highest_likelihood', 'largest_size', 'smallest_size', 'greatest_displacement', 'least_displacement', 'first_detected', 'last_detected', 'medicine_ball', or 'motion_specific'. `medicine_ball` keeps only the people present in at least 95% of frames and ranks them by distance to the selected ball during the first 10 frames. `motion_specific` ranks candidates by broad-jump or sprint-start motion features. Broad-jump still gates out short-lived, low-confidence, or too-small pose tracks, requires a flight interval with horizontal movement, and resolves ties by largest pose bbox. Sprint-start instead finds each track's maximal hip-normalized horizontal speed frame and evaluates the previous 1 second window, so short-lived runners are not rejected by whole-video presence/bbox gates; it then requires fast horizontal movement plus alternating heel-y oscillation. `on_click` is default and lets the user click on the persons they are interested in, in the desired order.
