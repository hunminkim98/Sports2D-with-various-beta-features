...
def get_pose2D(path, output_dir, type):
    ...
    print('\nGenerating 2D pose...')
    keypoints, scores = hrnet_pose(path, num_peroson=1, gen_output=True, type=type)
...
def img2video(video_path, filename, output_dir):
    ...
    names = sorted(glob.glob(os.path.join(output_dir + 'pose/', '*.jpg')))
...
def img2gif(video_path, name, output_dir, duration=0.25):
    ...
    image_list = sorted(glob.glob(os.path.join(output_dir + 'pose/', '*.jpg')))
