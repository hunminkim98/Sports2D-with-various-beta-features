...
DEFAULT_CONFIG =   {'base': {'video_input': ['demo.mp4'],
                    ...
                    'motion': {'vertical_jump': False,
                               'person_selection_target': 'auto',
                               'person_selection_presence_threshold': 0.8,
                               'person_selection_confidence_threshold': 0.3,
                               'person_selection_size_min_ratio': 0.35,
                               'person_selection_motion_score_threshold': 0.35},
...
CONFIG_HELP =   {'config': ["C", "path to a toml configuration file"],
                ...
                'person_ordering_method': ["", "'on_click', 'highest_likelihood', 'largest_size', 'smallest_size', 'greatest_displacement', 'least_displacement', 'first_detected', 'last_detected', 'medicine_ball', or 'motion_specific'. 'medicine_ball' keeps only the people present in at least 95%% of frames and ranks them by proximity to the selected ball over the first 10 frames. 'motion_specific' ranks broad_jump/sprint_start candidates; broad_jump uses whole-video presence/confidence/pose-bbox gates and requires an airborne interval with simultaneous horizontal motion, while sprint_start uses the 1 s window before peak hip-normalized horizontal speed and then requires fast horizontal motion with alternating heel-y oscillation. 'on_click' if not specified"],
                ...
                'person_selection_target': ["", "motion-specific person target: 'auto', 'broad_jump', 'sprint_start', or 'etc'. broad_jump requires airborne horizontal motion; sprint_start uses the 1 s window before peak hip-normalized horizontal speed and requires fast horizontal motion with alternating heel-y oscillation. Used only when person_ordering_method='motion_specific'. auto if not specified"],
                'person_selection_presence_threshold': ["", "minimum fraction of frames where a person track must be present before broad_jump/whole-video motion-specific ranking. Gate filter, not a ranking boost. sprint_start uses its local peak-speed window instead. 0.8 if not specified"],
                'person_selection_confidence_threshold': ["", "minimum mean keypoint confidence before motion-specific ranking. Gate filter, not a ranking boost. 0.3 if not specified"],
                'person_selection_size_min_ratio': ["", "minimum pose-bbox area ratio versus the largest candidate before motion-specific ranking. Gate filter to reject small background people. 0.35 if not specified"],
                'person_selection_motion_score_threshold': ["", "minimum broad_jump/sprint_start rule score needed after gate filtering. 0.35 if not specified"],
                ...
                'pose_model': ["p", "body_with_feet, whole_body_wrist, whole_body, body, synthpose, or synthpose_base. body_with_feet if not specified. synthpose uses VitPose-huge (52 keypoints mapped to HALPE_26), synthpose_base uses VitPose-base (faster but less accurate). Requires: pip install torch transformers"],
