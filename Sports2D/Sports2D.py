...
DEFAULT_CONFIG =   {'base': {'video_input': ['demo.mp4'],
                                ...
                                'save_pose': True,
                                'pose_3d': False,
                                'save_pose3d_comparison_vid': True,
                                'pose_3d_backend': 'fmpose3d',
                                ...
                                'fmpose3d_save_npz': False,
                                'fmpose3d_save_trc': True,
                                'fmpose3d_overwrite_m_trc': False,
                                'fmpose3d_num_samples': 1,
                                'fmpose3d_temporal_smooth_window': 1,
                                'fmpose3d_temporal_smooth_passes': 1,
                                'fmpose3d_stabilize_root_z': False,
                                'fmpose3d_root_z_window': 15,
                                'fmpose3d_seed': '',
                                'fmpose3d_smooth_profile': 'off',
...
CONFIG_HELP =   {'config': ["C", "path to a toml configuration file"],
                ...
                'pose_3d': ["", "run 2D-to-3D lifting from Sports2D keypoints. false if not specified"],
                'save_pose3d_comparison_vid': ["", "save side-by-side mp4 video (left: Sports2D overlay, right: FMPose3D 3D panel). true if not specified"],
                'pose_3d_backend': ["", "3D lifting backend. currently only 'fmpose3d' is supported"],
                'fmpose3d_checkpoint': ["", "path to FMPose3D checkpoint file (.pth)"],
                'fmpose3d_device': ["", "device for FMPose3D inference: auto, cuda, mps, or cpu"],
                'fmpose3d_sample_steps': ["", "number of Euler sampling steps for FMPose3D (>=1). 3 if not specified"],
                'fmpose3d_save_npz': ["", "save FMPose3D output as npz file. false if not specified"],
                'fmpose3d_save_trc': ["", "save FMPose3D output as TRC file (_fmpose3d suffix). true if not specified"],
                'fmpose3d_overwrite_m_trc': ["", "copy FMPose3D TRC to Sports2D compatibility path *_m_personXX.trc. false if not specified"],
                'fmpose3d_num_samples': ["", "number of stochastic FMPose3D runs to average (>=1). 1 if not specified"],
                'fmpose3d_temporal_smooth_window': ["", "temporal moving-average window for 3D output (odd int, >=1). 1 disables smoothing"],
                'fmpose3d_temporal_smooth_passes': ["", "number of repeated temporal smoothing passes (>=1). 1 if not specified"],
                'fmpose3d_stabilize_root_z': ["", "stabilize global vertical motion by smoothing root z trajectory. false if not specified"],
                'fmpose3d_root_z_window': ["", "moving-average window for root-z stabilization (odd int, >=1). 15 if not specified"],
                'fmpose3d_seed': ["", "optional deterministic seed for FMPose3D sampling. empty if not specified"],
                'fmpose3d_smooth_profile': ["", "preset for smoother 3D output: off, balanced, or max. off if not specified"],
